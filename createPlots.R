...
createPlots <- function() {
    ...
    avgTotalPayments <- dfNYPayments$Average.Total.Payments / 1000
    avgCoveredCharges <- dfNYPayments$Average.Covered.Charges / 1000
    ...
    plot(avgTotalPayments, avgCoveredCharges, 
         pch=20, type = "p", xlab="Mean Total Payments ($000's)", ylab="Mean Covered Charges ($000's)")
    ...
    text(x=13, y=130, labels = subtitleText)
    ...
    xRange = range(dfPayments$Average.Total.Payments)/1000
    yRange = range(dfPayments$Average.Covered.Charges)/1000
    # Setting to 0 to make the plots look clean; 
    ...
    devPDF <- pdf("plot2.pdf", width = 8, height  = 8)
    
    # Set the layout to 6 x 6, set internal and outer margins
    par(mfrow=c(length(states), length(DRG)), mar=c(0,0,0,0), oma=c(10,4,5,3))
    ...
    for(rowIdx in 1:length(states)){
        for(colIdx in 1:length(DRG)){
            ...
            avgTotalPayments <- dfPlot$Average.Total.Payments / 1000
            avgCoveredCharges <- dfPlot$Average.Covered.Charges / 1000
    ...
    mtext(text="Mean Total Payments ($000's)",side=1,cex=1,outer=TRUE,line=3)
    mtext(text="Mean Covered Charges ($000's)",side=2,cex=1,outer=TRUE,line=2)
    legend(x=-215, y=-110, legend=DRG, fill=DRG, xpd=NA, bty="n", ncol = 1)
